        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '就业分析名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.fenxiName" placeholder="就业分析名称" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '就业分析类型' : ''">
                        <el-select v-model="searchForm.fenxiTypes" placeholder="请选择就业分析类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in fenxiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('fenxi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/daxueshengjiuyefuwupingtai/upload/fenxiMuBan.xls"
                        >批量导入就业分析数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('fenxi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="fenxiUploadSuccess"
                                :on-error="fenxiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('fenxi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入就业分析数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('fenxi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "fenxi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="fenxiUuidNumber"
                                   header-align="center"
                                   label="就业分析编号">
                        <template slot-scope="scope">
                            {{scope.row.fenxiUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="fenxiPhoto"
                                     header-align="center"
                                     width="200"
                                     label="就业分析照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.fenxiPhoto">
                                <img :src="$base.url+scope.row.fenxiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('fenxi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('fenxi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('fenxi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              fenxiTypesSelectSearch : [],
            form:{
                id : null,
                fenxiName : null,
                fenxiUuidNumber : null,
                fenxiPhoto : null,
                fenxiTypes : null,
                fenxiQuxiang : null,
                fenxiXinzi : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'fenxi',//当前表表名,
                        // sumColum : 'fenxi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        string : 'fenxi_jiuyel',//分组字符串字段
                        // types : 'fenxi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.fenxiName!= '' && this.searchForm.fenxiName!= undefined) {
                    params['fenxiName'] = '%' + this.searchForm.fenxiName + '%'
                }
                         
                if (this.searchForm.fenxiTypes!= '' && this.searchForm.fenxiTypes!= undefined) {
                    params['fenxiTypes'] = this.searchForm.fenxiTypes
                }
                                                        
                params['fenxiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=fenxi_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.fenxiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            fenxiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "fenxi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
